import {cloneDeep, difference, get, isEmpty, orderBy, set} from 'lodash';
...
export const setSearchLeftMenu = (name: string, value: string) => async (dispatch: any, getState: any) => {
  ...
  if (value && value.length === 0) {
    ...
    const treeItemList = findTreeItemListByName(leftMenuData, value);
    console.log('treeItemList: ', treeItemList);
    const treeData = convertTreeDataByTreeItemList(cloneDeep(leftMenuData), treeItemList)
    console.log('treeData: ', treeData);
    const menuData = cloneMenuData(cloneDeep(leftMenuData))
    console.log('menuData: ', menuData);
    const newTreeData2 = createTreeItem(cloneDeep(leftMenuData), menuData, treeData)
...
function findTreeItemListByName(data: ILeftData[], name: string, nextData?: any) {
  ...
  for (let index = 0; index < data.length; index++) {
    ...
    if (element.display_name.toLocaleLowerCase().search(name.toLocaleLowerCase()) !== -1) {
      ...
      let newItem: any = findTreeItemListByName(element.children, name, outPut);
      if (newItem.length > 0) {
...
function cloneMenuData(data: ILeftData[], nextData?: ILeftData[]) {
  ...
  for (let index = 0; index < data.length; index++) {
    ...
    let newItem: any = cloneMenuData(element.children, outPut);
    if (newItem.length > 0) {
      outPut = newItem;
    }
  }
...
function convertTreeDataByTreeItemList(data: ILeftData[], pathFocus: any) {
...
function createTreeItem(data: ILeftData[] | any, menuData: ILeftData[], treeObject: any, nextData?: any) {
  ...
  for (let i = 0; i < treeObjectArr.length; i++) {
    ...
    const newItem = findTreeITem(data, menuData, path);
    console.log('path: ', path);
    console.log('newItem: ', newItem);
    if (value.children) {
      createTreeItem(data, value.children, outPut)
    }
  }
  console.log('====================================');
}
...
function findTreeITem(data: ILeftData[], menuData: ILeftData[], pathFocus: any[], nextData?: any) {
  let outPut: any = nextData || {};
  if (data.length === 0 || !pathFocus) {
    return {};
  }
  for (let index = 0; index < data.length; index++) {
    const element: ILeftData = data[index];
    if (difference(pathFocus, element.pathFocus).length === 0) {
      console.log('===========findTreeITem============');
      console.log('pathFocus: ', pathFocus);
      console.log('element.pathFocus: ', element.pathFocus);
      console.log('difference', difference(pathFocus, element.pathFocus));
      const data: any = get(menuData, element.pathFocus, {});
      set(outPut, pathFocus, data);
      break;
    } else {
      let newOutPut: any = findTreeITem(element.children, menuData, name, outPut);
